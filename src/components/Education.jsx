...
const EducationItem = ({ item, index }) => {
  ...
  return (
    <div className={`relative flex items-center justify-between mb-16 pl-16 pr-4 md:px-0 ${isEven ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
      {/* Content */}
      <motion.div
        ...
        className={`w-full md:w-[45%] bg-white/5 p-6 rounded-3xl border border-white/5 group hover:bg-white/10 hover:border-primary/40 hover:shadow-[0_0_20px] hover:shadow-primary/40 transition-all duration-300 ease-in-out`}
      >
      ...
      <motion.div
        ...
        className="absolute left-8 md:left-1/2 w-6 h-6 rounded-full glass border-2 border-primary z-10 flex items-center justify-center p-1"
        style={{ x: '-50%' }}
      >
...
const Education = () => {
  ...
  return (
    <section id="education" className="py-20 md:py-24 px-6 relative overflow-hidden" ref={containerRef}>
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-1 bg-white/5 rounded-full overflow-hidden" style={{ x: '-50%' }}>
            <motion.div
              ...
              className="absolute inset-0 bg-primary shadow-[0_0_15px] shadow-primary/30"
            />
          </div>
