...
const AutoCodingTerminal = () => {
    ...
    const fullCode = `> booting system...

const developer = {
  name: "Mahtab Alam",
  role: "Software Engineer",
  stack: ["PHP", "CakePHP", "React", "MySQL"]
};
...
✔ Deployment Successful`;
    ...
    useEffect(() => {
        ...
        const type = () => {
            if (!isDeleting && currentLength <= fullCode.length) {
                setTypedCode(fullCode.slice(0, currentLength));

                let delay = Math.random() * 10 + 30; // 30-40ms

                if (currentLength > 0) {
                    const lastChar = fullCode[currentLength - 1];
                    if (lastChar === '\n') delay = 300;
                    if (lastChar === '>' || lastChar === '✔') delay = 500;
            ...
            } else if (!isDeleting && currentLength > fullCode.length) {
                timeout = setTimeout(() => {
                    isDeleting = true;
                    type();
                }, 3500); // 3.5s delay
                ...
                setTypedCode(fullCode.slice(0, currentLength));
                currentLength--;
                timeout = setTimeout(type, 15); // 10-20ms deleting
            } else if (isDeleting && currentLength === 0) {
                isDeleting = false;
    ...
    const colorizeCode = (code) => {
        return code
            ...
            .replace(/\b(const|function|return)\b/g, '<span class="text-purple-400">$1</span>')
            .replace(/("[^"]*"?)/g, '<span class="text-green-400">$1</span>')
            .replace(/\b(developer|name|role|stack|deploy)\b/g, '<span class="text-blue-400">$1</span>')
    ...
    return (
        ...
        >
            <motion.div
                ...
                className="bg-[#020617] border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden"
            >
                ...
                <div className="flex items-center gap-2 mb-6 border-b border-white/10 pb-4 relative z-10">
                    ...
                    <span className="text-gray-400 text-xs font-mono ml-2">developer.js</span>
                </div>

                <div className="font-mono text-sm md:text-base text-gray-300 whitespace-pre relative z-10 min-h-[440px]">
...
const Skills = () => {
    return (
        <section id="skills" className="py-32 px-6 relative overflow-hidden flex items-center justify-center min-h-[90vh]">
